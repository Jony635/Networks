		...
		if (message == ClientMessage::Hello)
		{
			...
			if (proxy == nullptr)
			{
				...
				for (uint16 i = 0; i < networkGameObjectsCount; ++i)
				{
					...
					proxy->repManager.create(gameObject->networkId);
				}
	...
	if (state == ServerState::Listening)
	{
		...
		for (ClientProxy &clientProxy : clientProxies)
		{
			if (clientProxy.connected)
			{
				...
				//If the replication interval passed and the replication manager of this proxy
...
void ModuleNetworkingServer::onConnectionReset(const sockaddr_in & fromAddress)
{
	...
	if (proxy)
	{
		...
		for (int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			{
				//Notify this proxy's replication manager about the destruction of this player's game object
				clientProxies[i].repManager.destroy(proxy->gameObject->networkId);
			}
		}
...
GameObject * ModuleNetworkingServer::spawnPlayer(ClientProxy &clientProxy, uint8 spaceshipType)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		if (clientProxies[i].connected)
		{
			//Notify this proxy's replication manager about the creation of this game object
			clientProxies[i].repManager.create(clientProxy.gameObject->networkId);
		}
	}
...
GameObject * ModuleNetworkingServer::spawnBullet(GameObject *parent)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		{
			//Notify this proxy's replication manager about the creation of this game object
			clientProxies[i].repManager.create(gameObject->networkId);
		}
	}
...
void ModuleNetworkingServer::destroyNetworkObject(GameObject * gameObject)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		if (clientProxies[i].connected)
		{
			//Notify this proxy's replication manager about the destruction of this game object
			clientProxies[i].repManager.destroy(gameObject->networkId);
		}
	}
...
void ModuleNetworkingServer::updateNetworkObject(GameObject * gameObject)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		if (clientProxies[i].connected)
		{
			//Notify this proxy's replication manager about the update of this game object
			clientProxies[i].repManager.update(gameObject->networkId);
		}
	}
}
